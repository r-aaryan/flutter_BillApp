...
class BillService {
  ...
  /// Fetch bills for a user based on provider ID
  ...
  try {
    ...
    return snapshot.docs.map((doc) {
      return {
        ...
        'paymentStatus': doc['status'], // Match the field "status" in Firestore
      };
    }).toList();
  } catch (e) {
 ...
 Future<void> addBill({
  ...
  try {
    await _db.collection('bills').add({
      ...
      'createdAt': FieldValue.serverTimestamp(), // Timestamp of when the bill is created
    });
